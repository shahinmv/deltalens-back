...
existing_df = pd.read_csv('data/seconds/btc_second_ohlcv.csv', parse_dates=True, index_col='datetime')

# Get the last date in your CSV
last_date = existing_df.index[-1]
# Add 1 second instead of 1 hour to ensure continuous data
start_date = last_date + pd.Timedelta(seconds=1)
since_timestamp = int(start_date.timestamp())

# Fetch new data from Kraken API
def fetch_new_ohlc(pair='XBTUSD', interval=1, since=since_timestamp):
...
# Save updated data
full_df.to_csv('data/seconds/btc_second_ohlcv.csv')

print("✅ Data updated and saved to 'btc_second_ohlcv.csv'")
