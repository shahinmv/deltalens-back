    ...
    def simulate_trading(self, df, initial_capital=10000, transaction_cost=0.001):
        ...
        try:
            ...
            for i in range(len(ensemble_pred)):
                # Calculate trade start and end dates (30-day prediction horizon)
                if i < len(test_dates):
                    trade_start = test_dates[i]
                    # Find the end date (30 days later) in the original dataframe
                    try:
                        # Get the index position in the original dataframe
                        start_pos = df.index.get_loc(trade_start)
                        # Add 30 days (prediction horizon)
                        if start_pos + 30 < len(df):
                            trade_end = df.index[start_pos + 30]
                            btc_start_price = df.loc[trade_start, 'close'] if 'close' in df.columns else df.loc[trade_start, 'Close']
                            btc_end_price = df.loc[trade_end, 'close'] if 'close' in df.columns else df.loc[trade_end, 'Close']
                        else:
                            trade_end = df.index[-1]  # Use last available date
                            btc_start_price = df.loc[trade_start, 'close'] if 'close' in df.columns else df.loc[trade_start, 'Close']
                            btc_end_price = df.loc[trade_end, 'close'] if 'close' in df.columns else df.loc[trade_end, 'Close']
            ...
            for i in range(len(ensemble_pred)):
                ...
                trade_info = {
                    ...
                    'btc_return_actual': float((btc_prices_end[i] - btc_prices_start[i]) / btc_prices_start[i]) if btc_prices_start[i] > 0 else 0.0,
                    ...
                    'risk_adjusted_return': float(actual_return)
                }
                ...
                if abs(pred_return) > 0.02:  # Only trade if predicted return > 2%
                    if pred_return > 0:
                        ...
                        trade_info['stop_loss_triggered'] = risk_adjusted_return != actual_return
