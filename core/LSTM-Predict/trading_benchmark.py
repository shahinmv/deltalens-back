...
# Now import
from data_loader_copy import load_all_data
        ...
        try:
            ...
            if position_type == 'long':
                if momentum > 70:  # Overbought - tighter stops
                    ...
                    momentum_multiplier = 1.2  # Reduced from 1.3
                else:
                    momentum_multiplier = 1.0
            else:  # Short position - AGGRESSIVE LOGIC to favor shorts
                if momentum > 70:  # Overbought - highly favorable for shorts, very tight stops
                    momentum_multiplier = 0.6  # Even better for overbought conditions
                elif momentum > 60:  # Moderately overbought - still favorable
                    momentum_multiplier = 0.7  # Good conditions for shorts
                elif momentum < 30:  # Oversold - less favorable but not heavily penalized
                    momentum_multiplier = 0.95  # Minimal penalty
                else:
                    momentum_multiplier = 0.8  # Still favor shorts in neutral conditions
...
class IterativeTradingSystem:
    def __init__(self, 
        ...
        # SHORT signal boosting configuration - AGGRESSIVE BOOST
        self.boost_short_signals = self.config.get('boost_short_signals', True)
        self.short_threshold_multiplier = self.config.get('short_threshold_multiplier', 0.6)  # 40% easier for SHORT
        self.long_threshold_multiplier = self.config.get('long_threshold_multiplier', 1.2)  # 20% harder for LONG
        self.short_prediction_amplifier = self.config.get('short_prediction_amplifier', 1.3)  # 30% amplification for SHORT
    ...
    def _load_config(self) -> Dict:
        ...
        default_config = {
            ...
            # SHORT signal boosting parameters - AGGRESSIVE SETTINGS
            "boost_short_signals": True,
            "short_threshold_multiplier": 0.6,  # 40% easier for SHORT signals
            "long_threshold_multiplier": 1.2,  # 20% harder for LONG signals
            "short_prediction_amplifier": 1.3,  # 30% amplification for SHORT predictions
            "bear_market_threshold_reduced": 0.3,  # Further lowered for easier bear detection
            "short_confidence_boost": 1.2,  # 20% confidence boost for short signals
            "short_position_size_multiplier": 1.15  # 15% larger position sizes for shorts
        }
    ...
    def get_latest_date_from_ohlcv(self) -> Optional[str]:
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            query = "SELECT MAX(datetime) FROM btc_daily_copy"
            result = pd.read_sql_query(query, conn)['MAX(datetime)'].iloc[0]
            conn.close()
            return result
        except Exception as e:
    ...
    def get_yesterday_date(self) -> str:
        ...
        yesterday = self.get_latest_date_from_ohlcv()
        if yesterday is None:
            return None
        # Convert string to datetime if needed, then format
        if isinstance(yesterday, str):
            return yesterday[:10]  # Just take the YYYY-MM-DD part if it's already a string
        else:
    ...
    def generate_trading_signal(self, data: pd.DataFrame) -> Optional[TradeSignal]:
        ...
        try:
            ...
            if 'datetime' in data.columns:
                ...
                # DateTime index that might need conversion
                try:
                    # Convert index to string format for comparison
                    date_strings = [str(d)[:10] for d in data.index]
                    mask = [d == yesterday for d in date_strings]
                    yesterday_data = data[mask]
                    print(f"Yesterday data found via date conversion: {len(yesterday_data)} rows")
                    if len(yesterday_data) > 0:
                        yesterday_price = float(yesterday_data['close'].iloc[0])
                        print(f"Yesterday's close price: ${yesterday_price:.2f}")
                        logging.info(f"Found yesterday's ({yesterday}) close price: ${yesterday_price:.2f}")
                except Exception as e:
                    print(f"Error searching by date conversion: {e}")
            else:
            ...
            # Apply thresholds for signal generation (with AGGRESSIVE SHORT signal boosting)
            if self.boost_short_signals:
                # AGGRESSIVE asymmetric thresholds to heavily boost SHORT signals
                short_threshold = self.min_prediction_threshold * self.short_threshold_multiplier  # 1.2% for shorts (40% easier)
                long_threshold = self.min_prediction_threshold * self.long_threshold_multiplier    # 2.4% for longs (20% harder)
                
                # Apply additional amplification to the predicted return for SHORT consideration
                amplified_predicted_return = predicted_return
                if predicted_return < 0:  # Negative prediction (bearish)
                    amplified_predicted_return = predicted_return * self.short_prediction_amplifier  # Amplify bearish signals
                
                logging.info("🚀 AGGRESSIVE SHORT SIGNAL BOOSTING ENABLED")
                logging.info(f"   • SHORT threshold: {short_threshold:.4f} ({short_threshold*100:.2f}%)")
                logging.info(f"   • LONG threshold: {long_threshold:.4f} ({long_threshold*100:.2f}%)")
                logging.info(f"   • Amplified prediction: {amplified_predicted_return:.4f} (original: {predicted_return:.4f})")
            else:
                # Traditional symmetric thresholds
                short_threshold = self.min_prediction_threshold
                long_threshold = self.min_prediction_threshold
                amplified_predicted_return = predicted_return
                logging.info("⚖️ SYMMETRIC THRESHOLDS USED")
            
            # Use amplified prediction for signal generation when short boosting is enabled
            evaluation_return = amplified_predicted_return if self.boost_short_signals else predicted_return
            
            if evaluation_return < -short_threshold:  # SHORT signals (using amplified prediction)
                signal_type = 'SHORT'
                # Apply additional boosts for SHORT signals
                base_position_size = kelly_confidence
                if self.boost_short_signals:
                    # Boost confidence and position size for SHORT signals
                    short_confidence_boost = self.config.get('short_confidence_boost', 1.2)
                    short_position_multiplier = self.config.get('short_position_size_multiplier', 1.15)
                    kelly_confidence = min(kelly_confidence * short_confidence_boost, 0.15)  # Cap at 15%
                    position_size = min(base_position_size * short_position_multiplier, self.max_position_size)
                    logging.info(f"🚀 SHORT signal BOOSTED: confidence {base_confidence:.4f}→{kelly_confidence:.4f}, size {base_position_size:.4f}→{position_size:.4f}")
                else:
                    position_size = kelly_confidence
                logging.info(f"🔴 SHORT signal triggered: prediction={predicted_return:.4f}→{evaluation_return:.4f}, threshold={-short_threshold:.4f}")
            elif evaluation_return > long_threshold:  # LONG signals (using original or amplified prediction)
                signal_type = 'LONG'
                position_size = kelly_confidence
                logging.info(f"🟢 LONG signal triggered: prediction={predicted_return:.4f}, threshold={long_threshold:.4f}")
            else:
                signal_type = 'HOLD'
                position_size = 0.0
                logging.info(f"⭕ HOLD signal: prediction={predicted_return:.4f}→{evaluation_return:.4f}, short_thresh={-short_threshold:.4f}, long_thresh={long_threshold:.4f}")
    ...
    def save_signal_to_database(self, signal: TradeSignal, prediction_date: str) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS benchmark_trading_signals_v2 (
            ...
            cursor.execute("""
                INSERT INTO benchmark_trading_signals_v2 
    ...
    def run_production_prediction(self):
        ...
        logging.info("=" * 60)
        logging.info("Starting PRODUCTION prediction with DYNAMIC STOP LOSS & SHORT BOOST")
        if self.boost_short_signals:
            logging.info("🚀 SHORT SIGNAL IMPROVEMENTS ACTIVE:")
            logging.info(f"   • Asymmetric thresholds: SHORT {self.short_threshold_multiplier:.2f}x vs LONG {self.long_threshold_multiplier:.2f}x")
            logging.info(f"   • SHORT prediction amplification: {self.short_prediction_amplifier:.2f}x")
            logging.info(f"   • Enhanced bearish features included")
            logging.info(f"   • Improved dynamic stop loss for SHORT positions")
            logging.info(f"   • Lower bear market detection threshold")
        else:
            logging.info("⚖️ Traditional symmetric thresholds in use")
        
        try:
            # Get the latest date from OHLCV and add 1 day for prediction
            latest_date_str = self.get_latest_date_from_ohlcv()

            latest_date = pd.to_datetime(latest_date_str)
            today = (latest_date + timedelta(days=1)).strftime('%Y-%m-%d')
...
START_DATE = datetime.strptime("2023-08-15", "%Y-%m-%d")
END_DATE = datetime.strptime("2025-07-01", "%Y-%m-%d")
DB_PATH = '../../db.sqlite3'
SOURCE_TABLE = 'btc_daily_ohlcv'
DEST_TABLE = 'btc_daily_copy'
...
if __name__ == "__main__":
    ...
    # --- Create destination table if it doesn't exist (with same schema as source) ---
    cursor.execute(f"DELETE FROM {DEST_TABLE}")
    print(f"Cleared all existing data from {DEST_TABLE}")
    ...
    print(f"Inserted all data before {START_DATE.date()}")

    conn.commit()

    # --- Loop through each date and copy matching rows ---
    current_date = START_DATE
    while current_date <= END_DATE:
        # Close the connection temporarily to avoid database lock
        conn.close()
        ...
        # Reopen the connection for data copying
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        print(f"Copied data for {date_str}")
        conn.commit()
