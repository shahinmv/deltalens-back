...
class IterativeTradingSystem:
    ...
    def get_latest_date_from_copy_table(self) -> Optional[str]:
        """Get the latest date from btc_daily_copy table"""
        try:
            ...
            logging.error(f"Error getting latest date from copy table: {e}")
            return None
    
    def add_day_data(self, date_to_add: str) -> bool:
        """Add specified day's data from btc_daily_ohlcv to btc_daily_copy"""
        try:
            conn = sqlite3.connect(self.db_path)
            
            # Check if data exists in main table
            check_query = "SELECT * FROM btc_daily_ohlcv WHERE datetime = ?"
            day_data = pd.read_sql_query(check_query, conn, params=[date_to_add])
            
            if len(day_data) == 0:
                logging.warning(f"No data available for {date_to_add} in btc_daily_ohlcv")
                conn.close()
                return False
            
            # Insert day data into copy table
            day_data.to_sql('btc_daily_copy', conn, if_exists='append', index=False)
            
            conn.close()
            logging.info(f"Added data for {date_to_add} to btc_daily_copy")
            return True
            
        except Exception as e:
            logging.error(f"Error adding day data: {e}")
            return False
    
    def load_and_prepare_data_from_copy(self) -> Optional[pd.DataFrame]:
        """Load and prepare data using load_all_data function"""
        try:
            logging.info("Loading and preparing data using load_all_data function...")
            
            # Use the load_all_data function which should now use btc_daily_copy
            btc_ohlcv, daily_oi, daily_funding_rate, df_news = load_all_data()
            
            if btc_ohlcv is None or len(btc_ohlcv) == 0:
                logging.error("No Bitcoin OHLCV data loaded from btc_daily_copy")
                return None
            
            logging.info(f"Loaded {len(btc_ohlcv)} OHLCV records from btc_daily_copy")
            ...
            logging.error(f"Error loading and preparing data from copy table: {e}")
            return None
    ...
    def generate_trading_signal(self, data: pd.DataFrame) -> Optional[TradeSignal]:
        ...
        try:
            ...
            # Get current price and date for signal generation
    ...
    def run_iterative_prediction(self, start_date: str = "2025-05-01", end_date: str = "2025-05-30"):
        """Run iterative prediction with dynamic stop losses"""
        logging.info("=" * 60)
        logging.info(f"Starting iterative prediction with DYNAMIC STOP LOSS from {start_date} to {end_date}")
        ...
        try:
            while True:
                ...
                if current_dt >= end_dt:
                    logging.info(f"Reached end date {end_date}. Stopping.")
                    break
                
                logging.info("=" * 40)
                logging.info(f"Processing date: {current_date}")
                
                # 1. Load and prepare data up to current date
                data = self.load_and_prepare_data_from_copy()
                if data is None:
                    logging.error(f"Failed to load data for {current_date}")
                    break
                
                # 2. Train model with available data (includes dynamic stop loss calculator initialization)
                if not self.train_model(data):
                    logging.error(f"Failed to train model for {current_date}")
                    break
                
                # 3. Generate signal for current day with dynamic stop loss (using previous day's close)
                signal = self.generate_trading_signal(data)
                if signal is None:
                    logging.error(f"Failed to generate signal for {current_date}")
                    break
                
                # 4. Save enhanced signal to database
                if not self.save_signal_to_database(signal, current_date):
                    logging.error(f"Failed to save signal for {current_date}")
                    break
                
                # Store signal for analysis with dynamic stop loss info
                signal_dict = {
                    ...
                    'dynamic_stop_pct': signal.dynamic_stop_pct,
                    'market_volatility': signal.market_volatility,
                    'trend_strength': signal.trend_strength,
                    'momentum': signal.momentum,
                    'fear_index': signal.fear_index
                }
                self.all_signals.append(signal_dict)
                
                logging.info(f"Generated signal for {current_date}: {signal.signal_type} "
                           f"(return: {signal.predicted_return:.4f}, confidence: {signal.confidence:.4f}, "
                           f"dynamic_stop: {signal.dynamic_stop_pct*100:.1f}%)")
                
                # 5. Add current day's data to the copy table for next iteration
                if not self.add_day_data(current_date):
                    logging.error(f"Failed to add data for {current_date}. Stopping.")
                    break
            ...
            logging.info("=" * 60)
            logging.info("Iterative prediction with dynamic stop loss completed!")
            self.print_enhanced_summary()
            
        except Exception as e:
            logging.error(f"Error in iterative prediction: {e}")
...
if __name__ == "__main__":
    # Initialize enhanced iterative trading system with dynamic stop loss
    iterative_system = IterativeTradingSystem()
    
    # Run iterative prediction with dynamic stop loss from 2025-05-30 to 2025-06-30
    iterative_system.run_iterative_prediction(start_date="2025-05-01", end_date="2025-06-30")