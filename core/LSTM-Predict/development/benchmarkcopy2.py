...
class ComprehensiveTradingModelTester:
    ...
    def simulate_trading(self, df, initial_capital=10000, transaction_cost=0.001):
        """
        Simulate realistic trading with actual future prices for exits
        """
        try:
            # Prepare data for features only (don't use artificial targets)
            df_proc = self.predictor.engineer_30day_target(df)
            features, _ = self.predictor.prepare_features(df_proc)
            
            # Use actual prices and dates
            bitcoin_prices = df_proc['close'].values
            trade_dates = df_proc.index.values
            ...
            # Get corresponding Bitcoin prices and dates for test period
            sequence_length = X.shape[1] if len(X.shape) > 1 else 1
            test_price_start_idx = split_idx + sequence_length - 1
            
            # Ensure we have enough future data for 30-day exits
            prediction_horizon = 30  # 30 days
            if test_price_start_idx + len(X_test) + prediction_horizon > len(bitcoin_prices):
                # Trim test data to ensure we have future prices for all trades
                max_test_length = len(bitcoin_prices) - test_price_start_idx - prediction_horizon
                if max_test_length <= 0:
                    print("  Warning: Not enough data for 30-day forward testing")
                    return {
                ...
                X_test = X_test[:max_test_length]
            
            bitcoin_prices_test = bitcoin_prices[test_price_start_idx:test_price_start_idx + len(X_test)]
            trade_dates_test = trade_dates[test_price_start_idx:test_price_start_idx + len(X_test)]
            ...
            def check_exit_conditions(entry_price, current_price, position_type, days_held, predicted_return):
                """Check if trade should be closed based on price targets or time"""
                price_change = (current_price - entry_price) / entry_price
                target_return = abs(predicted_return)  # Use predicted return as take profit target
                
                # For long positions
                if position_type == 'long':
                    if price_change >= target_return:
                        return 'take_profit', target_return
                    elif price_change <= -stop_loss_pct:
                        return 'stop_loss', -stop_loss_pct
                    elif days_held >= 30:  # Auto close after 1 month
                        return 'auto_close', price_change
                
                # For short positions  
                elif position_type == 'short':
                    if price_change <= -target_return:  # Price drops for short profit
                        return 'take_profit', target_return
                    elif price_change >= stop_loss_pct:  # Price rises for short loss
                        return 'stop_loss', -stop_loss_pct
                    elif days_held >= 30:  # Auto close after 1 month
                        return 'auto_close', -price_change  # Negative for short
                
                return None, price_change
            
            # Simulate trading with dynamic exits
            capital = initial_capital
            positions = []
            returns = []
            ...
            for i in range(len(ensemble_pred)):
                current_date = trade_dates_test[i] if i < len(trade_dates_test) else None
                current_price = bitcoin_prices_test[i]
                
                # Check existing open trades for exit conditions
                trades_to_close = []
                for trade_idx, open_trade in enumerate(open_trades):
                    days_held = i - open_trade['entry_idx']
                    exit_reason, exit_return = check_exit_conditions(
                        open_trade['entry_price'], 
                        current_price, 
                        open_trade['type'], 
                        days_held,
                        open_trade['predicted_return']
                    )
                    
                    if exit_reason:
                        # Close the trade
                        position_value = open_trade['position_value']
                        if open_trade['type'] == 'long':
                            trade_return = position_value * exit_return
                        else:  # short
                            trade_return = position_value * exit_return
                        
                        exit_cost = position_value * transaction_cost
                        capital += trade_return - exit_cost
                        
                        # Record completed trade
                        trades.append({
                            'trade_id': open_trade['trade_id'],
                            'type': open_trade['type'],
                            'trade_date': str(current_date) if current_date is not None else None,
                            'trade_open_date': open_trade['trade_open_date'],
                            'trade_exit_date': str(current_date) if current_date is not None else None,
                            'predicted_return': open_trade['predicted_return'],
                            'actual_return': exit_return,
                            'risk_adjusted_return': exit_return,
                            'position_size': open_trade['position_size'],
                            'position_value': position_value,
                            'entry_cost': open_trade['entry_cost'],
                            'exit_cost': exit_cost,
                            'trade_return': trade_return,
                            'capital_before': capital - trade_return + exit_cost,
                            'capital_after': capital,
                            'exit_reason': exit_reason,
                            'days_held': days_held,
                            'stop_loss_triggered': exit_reason == 'stop_loss',
                            'take_profit_triggered': exit_reason == 'take_profit',
                            'auto_closed': exit_reason == 'auto_close',
                            'btc_entry_price': open_trade['entry_price'],
                            'btc_exit_price': current_price
                        })
                ...
                # Determine if we should open a new trade
                if abs(pred_return) > 0.02:  # Only trade if predicted return > 2%
                    ...
                    trade_type = 'long' if pred_return > 0 else 'short'
                    
                    # Open new trade
                    open_trades.append({
                        'trade_id': len(trades) + len(open_trades),
                        'type': trade_type,
                        'entry_idx': i,
                        'entry_price': current_price,
                        'trade_open_date': str(current_date) if current_date is not None else None,
                    ...
                    capital -= entry_cost  # Deduct entry cost
            ...
            final_price = bitcoin_prices_test[-1] if len(bitcoin_prices_test) > 0 else current_price
            for open_trade in open_trades:
                days_held = len(ensemble_pred) - 1 - open_trade['entry_idx']
                price_change = (final_price - open_trade['entry_price']) / open_trade['entry_price']
                ...
                trades.append({
                    'trade_id': open_trade['trade_id'],
                    'type': open_trade['type'],
                    'trade_date': 'end_of_period',
                    'trade_open_date': open_trade['trade_open_date'],
                    'trade_exit_date': 'end_of_period',
                    'predicted_return': open_trade['predicted_return'],
                    'actual_return': price_change if open_trade['type'] == 'long' else -price_change,
                    'risk_adjusted_return': price_change if open_trade['type'] == 'long' else -price_change,
                    'position_size': open_trade['position_size'],
                    'position_value': position_value,
                    'entry_cost': open_trade['entry_cost'],
                    'exit_cost': exit_cost,
            ...
            results = {
                ...
                'final_capital': capital,
                'total_return': total_return,
                'annualized_return': total_return * 252/30 / len(X_test) if len(X_test) > 0 else 0,
                'sharpe_ratio': sharpe,
                'max_drawdown': max_drawdown,
                'n_trades': n_trades,
                'win_rate': win_rate,
                'avg_trade_return': avg_trade_return,
                'trade_frequency': n_trades / len(X_test) if len(X_test) > 0 else 0,
                ...
                'trades': trades  # All trade details
            }
