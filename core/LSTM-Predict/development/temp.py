...
class RegimeAwareBitcoinPredictor:
    ...
    def build_regime_aware_model(self, input_shape):
        """Ultra-regularized regime-aware model to prevent overfitting"""
        inputs = layers.Input(shape=input_shape)
        
        # Add input noise for regularization
        inputs_noisy = layers.GaussianNoise(0.05)(inputs)
        
        # First LSTM with maximum regularization
        lstm1 = layers.LSTM(32, return_sequences=True,  # Reduced from 64
                        dropout=0.7,  # Increased from 0.5
                        recurrent_dropout=0.6,  # Increased from 0.4
                        kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.03, l2=0.03),  # Increased
                        recurrent_regularizer=tf.keras.regularizers.l2(0.02),  # Added
                        activity_regularizer=tf.keras.regularizers.l1(0.01))(inputs_noisy)  # Added
        lstm1 = layers.BatchNormalization()(lstm1)
        lstm1 = layers.Dropout(0.4)(lstm1)  # Additional dropout
        
        # Second LSTM with even stronger regularization
        lstm2 = layers.LSTM(16,  # Reduced from 32
                        dropout=0.7, 
                        recurrent_dropout=0.6,
                        kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.03, l2=0.03),
                        recurrent_regularizer=tf.keras.regularizers.l2(0.02),
                        activity_regularizer=tf.keras.regularizers.l1(0.01))(lstm1)
        lstm2 = layers.BatchNormalization()(lstm2)
        lstm2 = layers.Dropout(0.5)(lstm2)  # Additional dropout
        
        # First dense layer with maximum regularization
        dense1 = layers.Dense(24,  # Reduced from 64
                            activation='relu',
                            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.03, l2=0.03),  # Increased
                            activity_regularizer=tf.keras.regularizers.l2(0.02))(lstm2)  # Added
        dense1 = layers.Dropout(0.8)(dense1)  # Increased from 0.6
        dense1 = layers.BatchNormalization()(dense1)
        dense1 = layers.GaussianDropout(0.3)(dense1)  # Additional regularization
        
        # Second dense layer with ultra-strong regularization
        dense2 = layers.Dense(12,  # Reduced from 32
                            activation='relu',
                            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.04, l2=0.04),  # Increased
                            activity_regularizer=tf.keras.regularizers.l2(0.03))(dense1)
        dense2 = layers.Dropout(0.8)(dense2)  # Increased from 0.5
        dense2 = layers.BatchNormalization()(dense2)
        dense2 = layers.AlphaDropout(0.3)(dense2)  # Additional regularization
        
        # Output layer with maximum regularization and range limiting
        output = layers.Dense(1, activation='tanh',  # Changed from linear to tanh
                            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.02, l2=0.02),
                            activity_regularizer=tf.keras.regularizers.l2(0.01))(dense2)
        
        # Limit output range to prevent extreme predictions
        output = layers.Lambda(lambda x: x * 0.05)(output)  # Limit to ±5%
        
        model = Model(inputs=inputs, outputs=output)
        model.compile(
            optimizer=tf.keras.optimizers.Adam(
                learning_rate=0.0002,  # Reduced from 0.0005
                clipnorm=0.3,  # Reduced from 1.0
                beta_1=0.8,  # Reduced momentum
                beta_2=0.95,  # Reduced second momentum
                epsilon=1e-8,
                weight_decay=0.01  # Added weight decay
            ),
            loss=tf.keras.losses.Huber(delta=0.02),  # Reduced from 0.05
            metrics=['mae']
        )
    ...
    def build_bear_market_model(self, input_shape):
        """Ultra-conservative model for bear market conditions with maximum regularization"""
        inputs = layers.Input(shape=input_shape)
        
        # Add significant input noise
        inputs_noisy = layers.GaussianNoise(0.1)(inputs)
        
        # Single LSTM with extreme regularization
        lstm = layers.LSTM(16,  # Reduced from 32
                        dropout=0.8,  # Increased from 0.6
                        recurrent_dropout=0.7,  # Increased from 0.5
                        kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.05, l2=0.05),  # Increased
                        recurrent_regularizer=tf.keras.regularizers.l2(0.03),
                        activity_regularizer=tf.keras.regularizers.l1(0.02))(inputs_noisy)
        lstm = layers.BatchNormalization()(lstm)
        lstm = layers.Dropout(0.6)(lstm)  # Additional dropout
        lstm = layers.GaussianDropout(0.4)(lstm)  # Additional noise
        
        # Minimal dense layer with extreme regularization
        dense = layers.Dense(8,  # Reduced from 16
                            activation='relu',
                            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.05, l2=0.05),  # Increased
                            activity_regularizer=tf.keras.regularizers.l2(0.03))(lstm)
        dense = layers.Dropout(0.9)(dense)  # Increased from 0.7
        dense = layers.BatchNormalization()(dense)
        dense = layers.AlphaDropout(0.5)(dense)  # Additional regularization
        
        # Ultra-conservative output with severe range limiting
        output = layers.Dense(1, activation='tanh',
                            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.03, l2=0.03),
                            activity_regularizer=tf.keras.regularizers.l2(0.02))(dense)
        
        # Extremely limited range for bear market predictions
        output = layers.Lambda(lambda x: x * 0.02)(output)  # Limit to ±2%
        
        model = Model(inputs=inputs, outputs=output)
        model.compile(
            optimizer=tf.keras.optimizers.Adam(
                learning_rate=0.0001,  # Reduced from 0.0003
                clipnorm=0.2,  # Reduced from 0.5
                beta_1=0.7,  # Reduced momentum
                beta_2=0.9,  # Reduced second momentum
                weight_decay=0.02  # Increased weight decay
            ),
            loss=tf.keras.losses.Huber(delta=0.01),  # Reduced from 0.03
            metrics=['mae']
        )
    ...
    def create_sequences(self, features, targets, regimes=None):
        """Create sequences with ultra-strict validation to prevent overfitting"""
        X, y, regime_seq = [], [], []
        
        if len(features) < self.sequence_length + self.prediction_horizon:
            return np.array([]), np.array([]), []
        
        # Calculate statistics for more conservative filtering
        ...
        for i in range(len(features) - self.sequence_length - self.prediction_horizon + 1):
            try:
                ...
                # Much more conservative outlier filtering
                if np.abs(target) > 0.15:  # Reduced from 0.3
                    continue
                
                # Additional filtering based on statistics
                if np.abs(target) > target_median * 3:  # Remove extreme outliers
                    continue
                
                # Check sequence stability - reject highly volatile sequences
                seq_variations = np.std(seq, axis=0)
                if np.any(seq_variations > 5):  # Reduced from 10
                    continue
                    
                # Check for sudden spikes in the sequence
                seq_diffs = np.diff(seq, axis=0)
                if np.any(np.abs(seq_diffs) > 3):  # Reject sequences with sudden jumps
                    continue
                
                # Reject sequences with too many consecutive same values (data quality issue)
                for col in range(seq.shape[1]):
                    unique_values = len(np.unique(seq[:, col]))
                    if unique_values < 3:  # Need at least 3 unique values
                        continue
                
                # Additional target validation
                if np.abs(target) > target_std * 2.5:  # More conservative than 3-sigma
                    continue
        ...
        print(f"Filtered out {len(targets) - len(X)} sequences due to quality issues")
        return np.array(X), np.array(y), regime_seq

    def train_ensemble(self, df, validation_split=0.2, epochs=30, batch_size=32):  # Reduced epochs
        """Train ensemble with maximum regularization and overfitting prevention"""
        print("Training ultra-regularized ensemble...")
        ...
        try:
            ...
            # Create sequences with ultra-strict filtering
            ...
            early_stopping = callbacks.EarlyStopping(
                ...
                min_delta=0.0005,  # Increased threshold
                mode='min'
            )
            
            reduce_lr = callbacks.ReduceLROnPlateau(
                monitor='val_loss', 
                factor=0.2,  # More aggressive from 0.3
                patience=3,  # Reduced from 5
                min_lr=0.00001,  # Reduced minimum
                verbose=1
            )
            
            # Custom overfitting prevention callback
            class StrictOverfitCallback(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    if train_loss < val_loss * 0.7:  # More strict than 0.5
                        self.wait += 1
                        if self.wait >= self.patience:
                            print(f"\\nEarly stopping due to overfitting at epoch {epoch}")
                            self.model.stop_training = True
                    else:
                        self.wait = 0
            
            overfit_callback = StrictOverfitCallback(patience=2)
            
            # Train main regime-aware model with maximum regularization
            try:
                ...
                self.models['regime_aware'].fit(
                    ...
                    callbacks=[early_stopping, reduce_lr, overfit_callback],
                    verbose=0,
                    shuffle=False  # No shuffling for time series
                )
                print("Ultra-regularized regime-aware model trained")
                
            except Exception as e:
                print(f"Regime-aware model training failed: {e}")
            
            # Train bear market specialist with extreme regularization
            try:
                ...
                ultra_early_stopping = callbacks.EarlyStopping(
                    ...
                    restore_best_weights=True,
                    min_delta=0.001  # Increased threshold
                )
                
                self.models['bear_specialist'].fit(
                    X_train, y_train,
                    validation_data=(X_val, y_val),
                    epochs=epochs // 2,  # Reduced training time
                    batch_size=batch_size * 2,  # Larger batches for stability
                    callbacks=[ultra_early_stopping, reduce_lr, overfit_callback],
                    verbose=0,
                    shuffle=False
                )
                print("Extremely regularized bear specialist trained")
                
            except Exception as e:
                print(f"Bear specialist training failed: {e}")
            
            # Train ultra-conservative Random Forest
            try:
                X_train_flat = X_train.reshape(len(X_train), -1)
                X_val_flat = X_val.reshape(len(X_val), -1)
                
                # Maximum regularization Random Forest
                ...
                print("Ultra-conservative Random Forest trained")
                
            except Exception as e:
                print(f"Random Forest training failed: {e}")
            
            # Train meta-model with extreme regularization
            if len(self.models) > 1:
                self._train_ultra_conservative_meta_model(X_val, y_val)
