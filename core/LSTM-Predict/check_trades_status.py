    ...
    def update_trade_status(self, signal, exit_price, hit_target, hit_stop_loss, is_expired, exit_date, days_active, has_ohlcv_data):
        ...
        try:
            ...
            if signal['trade_status_id']:
                cursor.execute("""
                    UPDATE trade_status 
                    SET status = ?, entry_date = ?, current_price = ?, hit_target = ?, hit_stop_loss = ?, 
                        is_expired = ?, pnl_percentage = ?, exit_reason = ?, 
                        exit_price = ?, exit_date = ?, exit_timestamp = ?, 
                        days_active = ?, last_checked = ?, updated_at = ?
                    WHERE signal_id = ?
